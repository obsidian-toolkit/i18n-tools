...
export async function getData(locale) {
    ...
    const locales = locale
        ? from([locale])
        : from(readdirSync(LOCALE_DIR, { withFileTypes: true })).pipe(filter((dirent) => dirent.isDirectory() && dirent.name !== 'en'), map((dirent) => dirent.name));
    const results = { locales: [], enFlat: enFlat };
