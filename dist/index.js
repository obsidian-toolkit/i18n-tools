...
const AllLocalesView = ({ data }) => {
  ...
  return jsxs(Box, { flexDirection: "column", children: [jsx(Text, { children: "\u{1F4CA} Translation Progress Summary:" }), jsxs(Text, { color: "green", children: [" ", "English: ", totalKeys, "/", totalKeys, " (100%) \u2705"] }), sortedLocales.map((locale) => {
    ...
    return jsx(Text, { children: `  ${locale.locale}:${padding}${locale.completed}/${totalKeys} (${locale.percentage}%) ${statusIcon}${issuesNote}` }, locale.locale);
  }), jsxs(Box, { flexDirection: "column", children: [jsx(Text, { children: "\u{1F4CA} Translation Stats:" }), jsx(Text, { children: "\u2550".repeat(40) })] }), sortedLocales.map((locale) => jsx(LocaleDetailView, { locale, totalKeys, enFlat }, locale.locale))] });
};
const LocaleDetailView = ({ locale, totalKeys, enFlat }) => {
  const statusIcon = locale.percentage >= 80 ? "\u2705" : locale.percentage >= 50 ? "\u{1F7E1}" : "\u{1F534}";
  return jsxs(Box, { flexDirection: "column", marginBottom: 1, children: [jsxs(Text, { children: [statusIcon, " Locale ", locale.locale, ": ", locale.completed, "/", totalKeys, " keys (", locale.percentage, "%) ", statusIcon] }), locale.missing.length > 0 && jsxs(Box, { flexDirection: "column", marginLeft: 2, children: [jsxs(Text, { color: "red", children: ["\u274C Missing keys (", locale.missing.length, "):"] }), jsx(MissingKeysView, { missing: locale.missing })] }), locale.untranslated.length > 0 && jsxs(Box, { flexDirection: "column", marginLeft: 2, children: [jsxs(Text, { color: "yellow", children: ["\u{1F504} Untranslated values (", locale.untranslated.length, "):"] }), locale.untranslated.slice(0, 5).map(([key]) => jsxs(Text, { color: "gray", children: [" ", "- ", key, ': "', enFlat[key], '"'] }, key)), locale.untranslated.length > 5 && jsxs(Text, { color: "gray", children: [" ", "... and ", locale.untranslated.length - 5, " more"] })] }), locale.extra.length > 0 && jsxs(Box, { flexDirection: "column", marginLeft: 2, children: [jsxs(Text, { color: "cyan", children: ["\u26A0\uFE0F Extra keys (", locale.extra.length, "):"] }), jsxs(Text, { color: "gray", children: [" ", locale.extra.map(([k]) => k).join(", ")] })] })] });
};
const MissingKeysView = ({ missing }) => {
  ...
  return jsx(Box, { flexDirection: "column", children: Array.from(keysBySection.entries()).map(([section, keys]) => {
    if (keys.length === 1) {
      return jsxs(Text, { color: "gray", children: [" ", "- ", keys[0]] }, section);
    } else if (keys.length <= 3) {
      return jsx(Box, { flexDirection: "column", children: keys.map((key) => jsxs(Text, { color: "gray", children: [" ", "- ", key] }, key)) }, section);
    } else {
      return jsxs(Box, { flexDirection: "column", children: [jsxs(Text, { color: "gray", children: [" ", "- ", section, ".* (", keys.length, " keys missing)"] }), jsxs(Text, { color: "gray", children: [" ", "Examples:", " ", keys.slice(0, 2).map((k) => k.split(".").pop()).join(", "), "..."] })] }, section);
    }
  }) });
};
...
`);
  ...
  try {
    ...
    const untranslatedEntries = Array.from(localeActualMap).filter(([key, value]) => value === enFlat[key] || [value, enFlat[key]].every(Array.isArray) && JSON.stringify(value) === JSON.stringify(enFlat[key]));
...
async function getData(locale) {
  ...
  const enMap = new Map(Object.entries(enFlat));
  enMap.size;
  const locales = locale ? from([locale]) : from(readdirSync(LOCALE_DIR, { withFileTypes: true })).pipe(filter((dirent) => dirent.isDirectory() && dirent.name !== "en"), map((dirent) => dirent.name));
...
async function checkAllLocales() {
  ...
  render(jsx(AllLocalesView, { data }));
}
...
const LocaleView = ({ data }) => {
  ...
  const totalKeys = Object.keys(enFlat).length;
  if (locales.length === 0) {
    return jsx(Text, { color: "red", children: "\u274C No locale data found" });
  }
  const locale = locales[0];
  const statusIcon = locale.percentage >= 80 ? "\u2705" : locale.percentage >= 50 ? "\u{1F7E1}" : "\u{1F534}";
  return jsxs(Box, { flexDirection: "column", children: [jsxs(Text, { children: [statusIcon, " Locale ", locale.locale, ": ", locale.completed, "/", totalKeys, " keys (", locale.percentage, "%) ", statusIcon] }), locale.missing.length > 0 && jsxs(Box, { flexDirection: "column", marginTop: 1, children: [jsxs(Text, { color: "red", children: ["\u274C Missing keys (", locale.missing.length, "):"] }), locale.missing.slice(0, 10).map(([key]) => jsxs(Text, { color: "gray", children: [" ", "- ", key] }, key)), locale.missing.length > 10 && jsxs(Text, { color: "gray", children: [" ", "... and ", locale.missing.length - 10, " more"] })] }), locale.untranslated.length > 0 && jsxs(Box, { flexDirection: "column", marginTop: 1, children: [jsxs(Text, { color: "yellow", children: ["\u{1F504} Untranslated values (", locale.untranslated.length, "):"] }), locale.untranslated.slice(0, 5).map(([key]) => jsxs(Text, { color: "gray", children: [" ", "- ", key, ': "', enFlat[key], '"'] }, key)), locale.untranslated.length > 5 && jsxs(Text, { color: "gray", children: [" ", "... and ", locale.untranslated.length - 5, " more"] })] }), locale.extra.length > 0 && jsxs(Box, { flexDirection: "column", marginTop: 1, children: [jsxs(Text, { color: "cyan", children: ["\u26A0\uFE0F Extra keys (", locale.extra.length, "):"] }), jsxs(Text, { color: "gray", children: [" ", locale.extra.map(([k]) => k).join(", ")] })] }), jsx(Box, { marginTop: 1, children: jsxs(Text, { children: ["\u{1F4CA} Summary: ", locale.locale, ": ", locale.completed, "/", totalKeys, " ", "(", locale.percentage, "%)"] }) })] });
};

async function checkOneLocale(locale = "ru") {
  const data = await getData(locale);
  render(jsx(LocaleView, { data }));
}
...
const Error$1 = ({ message, exitApp = true }) => {
  ...
  return jsxs(Text, { color: "red", children: ["ERROR: ", message] });
};

const Success = ({ message }) => {
  const lines = Array.isArray(message) ? message : [message];
  return jsx(Box, { flexDirection: "column", children: lines.map((line, index) => jsxs(Text, { color: "green", children: [index === 0 ? "SUCCESS: " : "", line] }, index)) });
};
...
async function flat(locale) {
  ...
  if (!existsSync(tsPath)) {
    await render(jsx(Error$1, { message: `${tsPath} not found` })).waitUntilExit();
  }
  try {
    ...
    render(jsx(Success, { message: `Generated ${jsonPath}` }));
  } catch (error) {
    render(jsx(Error$1, { message: `Failed to process ${locale}: ${error.message}` }));
  }
}
...
async function* getLangStatsData() {
  ...
  const keys = from(project.getSourceFiles()).pipe(flatMap((file) => file.getDescendants()), filter((node) => node.getKind() === SyntaxKind.PropertyAccessExpression), filter((node) => node === getTopmostPropertyAccess(node)), map((node) => ({
    fullPath: node.getText(false).replace(/\s+/g, ""),
    node
  })), filter((nodeData) => nodeData.fullPath.startsWith("t.")), map(({ node, fullPath }) => ({
    node,
    key: fullPath.slice(2)
  })), tap(({ key }) => usedKeys.add(key)), map(({ node, key }) => ({
    key,
    value: enLocale[key],
  ...
  const unused = from(Object.keys(enLocale)).pipe(filter((key) => !usedKeys.has(key)), map((key) => ({
    key,
    value: enLocale[key],
    ctx: void 0,
    filePath: void 0,
...
const MainScreen = ({ data, message, selectedIndex, onNavigate, onRefresh, onExit }) => {
  ...
  return jsxs(Box, { flexDirection: "column", children: [jsx(Text, { children: "\u{1F4CA} Language Statistics" }), jsx(Text, { children: "\u2550".repeat(40) }), jsxs(Text, { children: ["\u{1F511} Total keys: ", data.totalKeys] }), jsxs(Text, { children: ["\u{1F4C8} Total usages: ", data.keysUsages] }), jsxs(Text, { children: ["\u{1F7E2} Used keys: ", data.usedKeysCount] }), jsxs(Text, { children: ["\u{1F534} Unused keys: ", data.unusedKeys] }), jsxs(Text, { children: ["\u{1F7E1} Missed keys: ", data.missedKeys] }), jsxs(Text, { children: ["\u{1F4CA} Usage: ", usedPercent, "% | Unused: ", unusedPercent, "%"] }), message && jsx(Text, { color: "green", children: message }), jsx(Text, { children: " " }), jsxs(Box, { flexDirection: "column", children: [jsx(Text, { children: "Actions:" }), menuItems.map((item, index) => jsxs(Text, { color: selectedIndex === index ? "cyan" : item.disabled ? "gray" : "white", children: [selectedIndex === index ? "> " : "  ", item.label] }, index))] }), jsx(Box, { marginTop: 1, children: jsx(Text, { dimColor: true, children: "\u2191\u2193 Navigate \u2022 Enter: Select \u2022 Q/Esc: Exit" }) })] });
};
const MissedScreen = ({ items, selectedIndex, message, onAutoResolve, onInteractive, onBack }) => {
  ...
  return jsxs(Box, { flexDirection: "column", children: [jsxs(Text, { children: ["\u{1F7E1} Missed Keys (", items.length, ")"] }), jsx(Text, { children: "\u2550".repeat(40) }), message && jsx(Text, { color: "green", children: message }), jsxs(Box, { flexDirection: "column", children: [jsx(Text, { children: "Actions:" }), menuItems.map((item, index) => jsxs(Text, { color: selectedIndex === index ? "cyan" : item.disabled ? "gray" : "white", children: [selectedIndex === index ? "> " : "  ", item.label] }, index))] }), jsx(Box, { marginTop: 1, children: jsx(Text, { dimColor: true, children: "\u2191\u2193 Navigate \u2022 Enter: Select \u2022 Esc: Back" }) })] });
};
const UnusedScreen = ({ items, selectedIndex, message, onAutoResolve, onInteractive, onBack }) => {
  ...
  return jsxs(Box, { flexDirection: "column", children: [jsxs(Text, { children: ["\u{1F534} Unused Keys (", items.length, ")"] }), jsx(Text, { children: "\u2550".repeat(40) }), message && jsx(Text, { color: "green", children: message }), jsxs(Box, { flexDirection: "column", children: [jsx(Text, { children: "Actions:" }), menuItems.map((item, index) => jsxs(Text, { color: selectedIndex === index ? "cyan" : item.disabled ? "gray" : "white", children: [selectedIndex === index ? "> " : "  ", item.label] }, index))] }), jsx(Box, { marginTop: 1, children: jsx(Text, { dimColor: true, children: "\u2191\u2193 Navigate \u2022 Enter: Select \u2022 Esc: Back" }) })] });
};
const InteractiveList = ({ items, onRemove, onBack }) => {
  ...
  if (items.length === 0) {
    return jsxs(Box, { flexDirection: "column", alignItems: "center", padding: 2, children: [jsx(Text, { color: "green", children: "\u2705 No items remaining!" }), jsx(Text, { color: "gray", children: "Press Esc to return" })] });
  }
  ...
  if (selectedIndex >= items.length) {
    return jsxs(Box, { flexDirection: "column", alignItems: "center", padding: 2, children: [jsx(Text, { color: "red", children: "Index out of bounds" }), jsx(Text, { color: "gray", children: "Press Esc to return" })] });
  }
  if (viewMode === "detail") {
    ...
    return jsxs(Box, { flexDirection: "column", padding: 1, children: [jsxs(Text, { color: "cyan", children: ["\u{1F527} Item Details (", selectedIndex + 1, "/", items.length, ")"] }), jsx(Text, { color: "gray", children: "\u2550".repeat(50) }), jsxs(Box, { flexDirection: "column", marginTop: 1, children: [jsxs(Box, { children: [jsx(Text, { color: "blue", children: "Key: " }), jsx(Text, { color: "white", children: item.key })] }), jsxs(Box, { marginTop: 1, children: [jsx(Text, { color: "blue", children: "Value: " }), jsx(Text, { children: renderValue(item.value) })] }), item.filePath && jsxs(Box, { marginTop: 1, children: [jsx(Text, { color: "blue", children: "File: " }), jsx(Text, { color: "gray", children: item.filePath })] }), item.ctx && jsxs(Box, { flexDirection: "column", marginTop: 1, children: [jsx(Text, { color: "blue", children: "Context:" }), jsx(Box, { marginLeft: 2, marginTop: 1, borderStyle: "single", borderColor: "gray", children: jsx(Text, { children: renderCodeContext(item.ctx) }) })] })] }), jsx(Text, { color: "gray", children: "S: Skip \u2022 R: Remove \u2022 Esc: Back to list" })] });
  }
  return jsxs(Box, { flexDirection: "column", padding: 1, children: [jsxs(Text, { color: "cyan", children: ["\u{1F527} Interactive Mode (", items.length, ")"] }), jsx(Text, { color: "gray", children: "\u2550".repeat(40) }), items.slice(0, 15).map((item, index) => jsx(Box, { children: jsxs(Text, { color: index === selectedIndex ? "yellow" : "white", children: [index === selectedIndex ? "> " : "  ", item.key] }) }, `${item.key}-${index}`)), items.length > 15 && jsxs(Text, { color: "gray", children: ["... and ", items.length - 15, " more"] }), jsx(Text, { color: "gray", children: "\u2191\u2193 Navigate \u2022 Enter: View details \u2022 Esc: Back" })] });
};
const App = () => {
  ...
  const unused = useMemo(() => state.collectedData ? Object.entries(state.collectedData.keys).filter(([, value]) => value.type === "unused").map(([key, value]) => value.occurrences[0]) : [], [state.collectedData]);
  const missed = useMemo(() => state.collectedData ? Object.entries(state.collectedData.keys).filter(([, value]) => value.type === "missed").map(([key, value]) => value.occurrences[0]) : [], [state.collectedData]);
  ...
  if (state.loading) {
    return jsx(Box, { flexDirection: "column", alignItems: "center", children: jsx(Text, { children: "\u{1F504} Loading data..." }) });
  }
  if (state.processing) {
    return jsx(Box, { flexDirection: "column", alignItems: "center", children: jsxs(Text, { children: ["\u26A1 ", state.message] }) });
  }
  if (!state.collectedData) {
    return jsx(Text, { color: "red", children: "Failed to load data" });
  }
  if (state.currentItems.length > 0) {
    return jsx(InteractiveList, { items: state.currentItems, onRemove: handleRemoveItem, onBack: () => setState((prev) => ({ ...prev, currentItems: [] })) });
  }
  if (state.screen === "main") {
    return jsx(MainScreen, { data: state.collectedData, message: state.message, selectedIndex: selectedIndexRef.current, onNavigate: handleNavigation, onRefresh: handleRefresh, onExit: exit });
  }
  if (state.screen === "unused") {
    return jsx(UnusedScreen, { items: unused, selectedIndex: selectedIndexRef.current, message: state.message, onAutoResolve: autoResolveUnused, onInteractive: () => handleInteractive(unused), onBack: () => handleNavigation("main") });
  }
  if (state.screen === "missed") {
    return jsx(MissedScreen, { items: missed, selectedIndex: selectedIndexRef.current, message: state.message, onAutoResolve: autoResolveMissed, onInteractive: () => handleInteractive(missed), onBack: () => handleNavigation("main") });
  }
  return null;
};
function showStatsMenu() {
  render(jsx(App, {}), { exitOnCtrlC: false });
}
...
`.trim();
    ...
    render(jsx(Success, { message: `\u2705 Generated ${SCHEMA_FILE}` }));
  } catch (error) {
    render(jsx(Error$1, { message: "\u274C Error generating types:" }));
  }
}
...
async function nest(locale) {
  ...
  if (!existsSync(jsonPath)) {
    render(jsx(Error$1, { message: `${jsonPath} not found` }));
  }
  try {
...
export default Locale;`;
    ...
    render(jsx(Success, { message: `\u2705 Generated ${tsPath}` }));
    if (locale === "en") {
      await generateSchemaFromNest(nested);
    }
    return;
  } catch (error) {
    render(jsx(Error$1, { message: `${error.message}` }));
  }
}
...
async function template(locale) {
  ...
  if (!existsSync(enJsonPath)) {
    await render(jsx(Error$1, { message: 'en/flat.json not found. Run "flat en" first.' })).waitUntilExit();
  }
  const newLocaleDir = join(LOCALE_DIR, locale);
  if (existsSync(newLocaleDir)) {
    await render(jsx(Error$1, { message: `Locale "${locale}" already exists` })).waitUntilExit();
  }
  try {
...
program.command("nest-all").description("Update nested TypeScript files for all locales from their flat.json").action(updateAllNested);
...
program.command("keys-stats").description("Show detailed statistics of translation keys across locales, including total, missing, and unused keys").action(keysStats);
program.parse();
